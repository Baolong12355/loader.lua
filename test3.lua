...
-- Đây là hàm nhắm kẻ địch ĐI ĐẦU (gần base)
...
for _, enemy in ipairs(getEnemies()) do
    ...
    if getDistance2D(ePos, pos) <= range then
        local currentPathDistance = getEnemyPathProgress(enemy)
        if currentPathDistance > maxPathDistance then -- Tìm giá trị LỚN NHẤT
...
local ePos = enemy:GetPosition()
if getDistance2D(ePos, pos) <= range then
table.insert(candidates, {enemy = enemy, position = ePos, pathDistance = getEnemyPathProgress(enemy)})
end
end
if #candidates == 0 then return nil end
-- Sắp xếp để tìm kẻ địch ĐI ĐẦU (gần base)
table.sort(candidates, function(a, b) return a.pathDistance > b.pathDistance end) -- Tìm giá trị LỚN NHẤT
...
table.sort(candidates, function(a, b)
    ...
    if maxHpA ~= maxHpB then
        ...
        local pathDistA = getEnemyPathProgress(a) or 0
        local pathDistB = getEnemyPathProgress(b) or 0
