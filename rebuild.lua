-- TDX Macro Runner - Rebuild (Fixed & Loadstring-Compatible with HP Check)
        ...
        task.spawn(function() 
            while rebuildActive do 
                for x, _ in pairs(placedTowers) do 
                    ...
                    if not t then 
                        dead = true 
                    elseif t.HealthHandler then 
                        local hp = t.HealthHandler:GetHealth() 
                        if hp <= 0 then 
                            dead = true 
                        end 
                    end 
                    if dead then 
                        print("[DETECT] Tower mất hoặc chết tại X:", x) 
                        placedTowers[x] = nil 
