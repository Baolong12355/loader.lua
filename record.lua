	...
	while true do
		for hash, tower in pairs(TowerClass.GetTowers()) do
			...
			if pos then
				...
				hash2tower[tostring(hash)] = tower
			end
		end
		task.wait(0.1)
	end
		...
		for line in macro:gmatch("[^\r\n]+") do
			...
			if a1 and name and x and y and z and rot then
				...
				if hash and path then
					local tower = hash2tower[tostring(hash)]
					local pos = hash2pos[tostring(hash)]
					path = tonumber(path)
					if tower and tower.LevelHandler and pos then
						local key = tostring(hash) .. ":" .. tostring(path)
						local oldLevel = lastLevelMap[key] or -1

						task.wait(0.15) -- ⏳ đợi cấp độ cập nhật
