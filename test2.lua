--- START OF FILE rebuild.lua (FINAL CLEAN VERSION - Priority-Only Queue) ---
...
-- HỆ THỐNG CHÍNH VỚI MÔ HÌNH 2 LUỒNG
task.spawn(function()
        ...
        task.spawn(function()
            while true do
                if #jobQueue > 0 then
                    ...
                    if placeRecord and not GetTowerByAxis(x) then
                    ...
                    if rebuildSuccess then
                        ...
                        for _, record in ipairs(upgradeRecords) do
                            if not UpgradeTower(tonumber(record.entry.TowerUpgraded), record.entry.UpgradePath) then rebuildSuccess = false; break end
                            task.wait(0.1)
                        end
                    end
    ...
    while true do
        ...
        if jobsAdded and #jobQueue > 1 then
            -- CẬP NHẬT: Logic sắp xếp mới, chỉ dựa vào độ ưu tiên
