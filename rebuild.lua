...
if not TowerClass then
    error("TowerClass load failed")
end

local debugLines = {}
local function LogDebug(...)
    local msg = "[" .. os.date("%X") .. "] " .. table.concat({...}, " ")
    print(msg)
    table.insert(debugLines, msg)
end

local function SaveDebugLog()
    local content = table.concat(debugLines, "\n")
    writefile("log_rebuild.txt", content)
end

function GetTowerByAxis(axisX)
    ...
    LogDebug("MISSING", axisX)
    return nil, nil, nil
end

function GetCurrentUpgradeCost(tower, path)
    ...
    if ok2 and typeof(d) == "number" then
        disc = d
    end
    return math.floor(baseCost * (1 - disc))
end

function WaitForCash(amount)
    while cashStat.Value < amount do
        task.wait()
    end
end

function PlaceTowerRetry(args, axisValue, towerName)
    while true do
        ...
        repeat
            ...
            if hash then
                LogDebug("PLACED", towerName, axisValue)
                return
            end
        until tick() - t0 > 2
        LogDebug("RETRY PLACE", towerName, axisValue)
    end
end

function UpgradeTowerRetry(axisValue, path)
    local tries = 0
    while true do
        ...
        repeat
            ...
            if t and t.LevelHandler:GetLevelOnPath(path) > before then
                LogDebug("UPGRADED", axisValue, path)
                return
            end
        until tick() - t0 > 2
        tries += 1
...
function ChangeTargetRetry(axisValue, targetType)
    while true do
        local hash = GetTowerByAxis(axisValue)
        if hash then
            Remotes.ChangeQueryType:FireServer(hash, targetType)
            LogDebug("TARGET", axisValue, targetType)
            return
        end
        task.wait()
    end
...
function SellTowerRetry(axisValue)
    while true do
        local hash = GetTowerByAxis(axisValue)
        if hash then
