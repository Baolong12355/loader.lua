...
local lastLevels = {} -- dùng để đối chiếu path được nâng
...
local function setPending(typeStr, code)
    table.insert(pendingQueue, { type = typeStr, code = code, created = tick() })
end

-- Hook remote xác thực server gửi về
...
ReplicatedStorage.Remotes.TowerUpgradeQueueUpdated.OnClientEvent:Connect(function(data)
    local towerInfo = data[1]
    if not towerInfo then return end

    for _, tower in ipairs(towerInfo) do
        local hash = tower.Hash
        local newLevels = tower.LevelReplicationData

        local oldLevels = lastLevels[hash]
        if oldLevels then
            for pathIndex = 1, #newLevels do
                if newLevels[pathIndex] > oldLevels[pathIndex] then
                    local code = string.format("TDX:upgradeTower(%s, %d, 1)", tostring(hash), pathIndex - 1)
...
local function handleRemote(name, args)
    ...
    elseif name == "PlaceTower" then
        ...
        if typeof(a1) == "number" and typeof(towerName) == "string" and typeof(vec) == "Vector3" and typeof(rot) == "number" then
            local code = string.format('TDX:placeTower(%d, "%s", Vector3.new(%s, %s, %s), %d)', a1, towerName, tostring(vec.X), tostring(vec.Y), tostring(vec.Z), rot)
...
print("✅ Full hook hoạt động kèm xác thực path nâng cấp từ server (LevelReplicationData)")
