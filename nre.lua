...
local pending = nil local timeout = 2 local isRebuilding = false local enableRebuild = true local soldAxes = {}
...
ReplicatedStorage.Remotes.TowerFactoryQueueUpdated.OnClientEvent:Connect(function(data) local d = data[1] if not d then return end if not d.Creation and d.Hash and d.Axis then soldAxes[tonumber(d.Axis)] = true end tryConfirm(d.Creation and "Place" or "Sell") end)
...
local oldNamecall = hookmetamethod(game, "__namecall", function(self, ...) local method = getnamecallmethod() if not isRebuilding and (method == "FireServer" or method == "InvokeServer") then local name = self.Name local args = serializeArgs(...) if name == "PlaceTower" then setPending("Place", "TDX:placeTower(" .. args .. ")") elseif name == "SellTower" then setPending("Sell", "TDX:sellTower(" .. args .. ")") elseif name == "TowerUpgradeRequest" then setPending("Upgrade", "TDX:upgradeTower(" .. args .. ")") elseif name == "ChangeQueryType" then setPending("Target", "TDX:changeQueryType(" .. args .. ")") end end return oldNamecall(self, ...) end)

local TowerClass = require(Players.LocalPlayer.PlayerScripts.Client.GameClass.TowerClass) local Remotes = ReplicatedStorage:WaitForChild("Remotes")
...
local function PlaceTowerRetry(args, axisValue) if soldAxes[axisValue] then return end while true do Remotes.PlaceTower:InvokeServer(unpack(args)) task.wait(0.1) if GetTowerByAxis(axisValue) then return end end end
...
pcall(function() local gui = Instance.new("ScreenGui", player:WaitForChild("PlayerGui")) gui.Name = "TDX_GUI" local frame = Instance.new("Frame", gui) frame.Size = UDim2.new(0, 160, 0, 50) frame.Position = UDim2.new(0, 20, 0, 60) frame.BackgroundColor3 = Color3.fromRGB(50, 50, 50) frame.Active = true frame.Draggable = true local button = Instance.new("TextButton", frame) button.Size = UDim2.new(1, 0, 1, 0) button.Text = "🔁 Rebuild: ON" button.BackgroundColor3 = Color3.fromRGB(40, 180, 40) button.MouseButton1Click:Connect(function() enableRebuild = not enableRebuild button.Text = enableRebuild and "🔁 Rebuild: ON" or "⛔ Rebuild: OFF" button.BackgroundColor3 = enableRebuild and Color3.fromRGB(40, 180, 40) or Color3.fromRGB(180, 40, 40) end) end)

print("📌 Macro Recorder + Rebuild Ready with proper hooks")
