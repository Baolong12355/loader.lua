...
local function joinLeastPopulatedServer()
    ...
    if success and result.data and #result.data > 0 then
        local leastPopulated = result.data[1]
        for _, server in ipairs(result.data) do
            if server.playerCount < leastPopulated.playerCount then
                leastPopulated = server
            end
        end
        
        if leastPopulated and leastPopulated.playerCount < 50 then
            pcall(function()
                TeleportService:TeleportToPlaceInstance(placeId, leastPopulated.id, game.Players.LocalPlayer)
            end)
            wait(5)
        end
    end
