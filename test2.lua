-- START OF FILE rebuild.lua (Optimized)
...
local function RebuildTowerSequence(records)
    ...
    -- Step 2: Apply moving skills ASAP when skills become available (theo runner system)
...
task.spawn(function()
    ...
    while true do
        ...
        -- ==== OPTIMIZED PRODUCER ====
        -- Create a cache of existing towers ONCE per frame for fast lookups
        local existingTowersCache = {}
        for hash, tower in pairs(TowerClass.GetTowers()) do
            if tower.SpawnCFrame and typeof(tower.SpawnCFrame) == "CFrame" then
                existingTowersCache[tower.SpawnCFrame.Position.X] = true -- Store a simple true value
            end
        end
        ...
        for x, records in pairs(towersByAxis) do
            ...
            if shouldProcessTower then
                -- NEW EFFICIENT CHECK using the cache
                if not existingTowersCache[x] then
                    -- Tower doesn't exist (dead OR sold)
                    if not activeJobs[x] then -- Not already being rebuilt
                        recordTowerDeath(x)
                        ...
                        if towerType then
                            ...
                            if not maxRetry or rebuildAttempts[x] <= maxRetry then
                                -- Add to queue (sorting will be done later)
                                activeJobs[x] = true
        ...
        -- Sort the entire queue once after checking all towers
        if #jobQueue > 1 then
