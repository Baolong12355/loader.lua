...
local function tryRun(playerName, name, enabled, url)
    ...
    local embed = {
        title = ok and "Function Loaded Successfully" or "Function Failed to Load",
        description = "Function **`" .. name .. "`** " .. (ok and "has been loaded" or "failed to load") .. " for user **`" .. playerName .. "`**.",
        color = ok and 3066993 or 15158332,
        fields = {
    ...
    if not ok then
        table.insert(embed.fields, { name = "Error Message", value = "```\n" .. tostring(result) .. "\n```" })
    end
    sendToWebhook(embed)
end
...
task.spawn(function()
    pcall(function()
        loadstring(game:HttpGet(links["Watcher"]))()
        sendToWebhook({
            title = "Watcher Loaded",
            description = "Watcher.lua has been loaded for user **`" .. player.Name .. "`**.",
            color = 3066993,
            footer = { text = "Loader Log" },
            timestamp = os.date("!%Y-%m-%dT%H:%M:%S.000Z")
        })
...
local function logUserConfigFull(configTable)
    ...
    local success, fullJson = pcall(function()
        return HttpService:JSONEncode(copy)
    end)
    local jsonText = success and fullJson or "Failed to encode config"
    local preview = jsonText:sub(1, 1000)
    if jsonText:len() > 1000 then
        preview = preview .. "...\n(Truncated)"
    end
    sendToWebhook({
...
-- safe delay runner
local function safeSpawn(delayTime, func)
    task.spawn(function()
        task.wait(delayTime)
        pcall(func)
    end)
...
-- join map
safeSpawn(delay, function()
    tryRun(player.Name, "Join Map", getgenv().TDX_Config["Map"] ~= nil, links["Join Map"])
end)
delay += delayGap

if not shouldSkipFeatures then
    safeSpawn(delay, function()
        tryRun(player.Name, "Return Lobby", getgenv().TDX_Config["Return Lobby"], links["Return Lobby"])
    end)
    delay += delayGap
    safeSpawn(delay, function()
        tryRun(player.Name, "x1.5 Speed", getgenv().TDX_Config["x1.5 Speed"], links["x1.5 Speed"])
    end)
    delay += delayGap
    safeSpawn(delay, function()
        tryRun(player.Name, "Auto Difficulty", getgenv().TDX_Config["Auto Difficulty"] ~= nil, links["Auto Difficulty"])
    end)
    delay += delayGap
    safeSpawn(delay, function()
        tryRun(player.Name, "Heal", getgenv().TDX_Config["Heal"], links["Heal"])
    end)
    delay += delayGap
    safeSpawn(delay, function()
        tryRun(player.Name, "Loadout", getgenv().TDX_Config["Loadout"], links["Loadout"])
    end)
    delay += delayGap
    safeSpawn(delay, function()
        tryRun(player.Name, "Voter", getgenv().TDX_Config["Voter"], links["Voter"])
    end)
    delay += delayGap
    safeSpawn(delay, function()
        tryRun(player.Name, "Auto Skill", getgenv().TDX_Config["Auto Skill"], links["Auto Skill"])
    end)
else
    sendToWebhook({
...
safeSpawn(delay, function()
    ...
    if macro_type == "run" or macro_type == "record" then
        ...
        tryRun(player.Name, macroName, true, links[macroName])
    end
end)
