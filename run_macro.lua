...
local function RebuildTowerAsync(x, records, rebuildAttempts, soldPositions, config)
    return task.spawn(function()
        ...
        for _, record in ipairs(records) do
            ...
            if action.TowerPlaced then
                ...
                if #vecTab == 3 then
                    ...
                    -- Đảm bảo tower đã được đặt thành công trước khi tiếp tục
                    local waitTime = 0
                    while not GetTowerByAxis(pos.X) and waitTime < 5 do
                        task.wait(0.1)
                        waitTime = waitTime + 0.1
                    end
                    
                    if not GetTowerByAxis(pos.X) then
                        rebuildSuccess = false
                        break
                    end
                end
            ...
            elseif action.TowerUpgraded then
                local axis = tonumber(action.TowerUpgraded)
                -- Đảm bảo tower tồn tại trước khi upgrade
                local hash, tower = GetTowerByAxis(axis)
                if hash and tower then
                    UpgradeTowerRetry(axis, action.UpgradePath)
                    
                    -- Đợi upgrade hoàn thành
                    local waitTime = 0
                    local initialLevel = tower.LevelHandler:GetLevelOnPath(action.UpgradePath)
                    while waitTime < 3 do
                        local _, currentTower = GetTowerByAxis(axis)
                        if currentTower and currentTower.LevelHandler:GetLevelOnPath(action.UpgradePath) > initialLevel then
                            break
                        end
                        task.wait(0.1)
                        waitTime = waitTime + 0.1
            ...
            elseif action.ChangeTarget then
                local axis = tonumber(action.ChangeTarget)
                -- Đảm bảo tower tồn tại trước khi change target
                local hash = GetTowerByAxis(axis)
                if hash then
                    ChangeTargetRetry(axis, action.TargetType)
                    task.wait(0.1) -- Small delay để đảm bảo target change được apply
                else
                    rebuildSuccess = false
                    break
                end
            ...
            elseif action.SellTower then
                local axis = tonumber(action.SellTower)
                local sellSuccess = SellTowerRetry(axis)
                if sellSuccess then
                    soldPositions[axis] = true
                    
                    -- Đợi tower được sell hoàn toàn
                    local waitTime = 0
                    while GetTowerByAxis(axis) and waitTime < 3 do
                        task.wait(0.1)
                        waitTime = waitTime + 0.1
                    end
                end
        ...
        if rebuildSuccess then
            ...
            -- Nếu rebuild fail, increment attempts nhưng không clear death
