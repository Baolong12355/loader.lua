...
-- ⚙️ Hàm kiểm tra thiếu tower local function IsTowerMissing(x) local _, tower = GetTowerByAxis(x) return not tower end

-- 🧠 Gộp dữ liệu từ macro (từ dòng đầu đến maxLine) local function BuildTowerRecords(macro, maxLine, skipList, beMode) local records = {} local skipSet = {} for _, name in ipairs(skipList or {}) do skipSet[name] = true end
...
-- 🔼 Ưu tiên theo loại tower local priorityMap = { Medic = 1, ["Golden Mobster"] = 2, Mobster = 2, DJ = 3, Commander = 4 }

local function GetPriority(name) return priorityMap[name] or 5 end

-- 🚀 Watcher loop chính function startRebuildWatcher(macro, maxLine, skipList, beMode) local towerRecords = BuildTowerRecords(macro, maxLine, skipList, beMode)
...
-- 🔄 Tự động kích hoạt watcher nếu gặp "SuperFunction": "rebuild" local function DetectRebuildEntry(macro) for i, entry in ipairs(macro) do if entry.SuperFunction == "rebuild" then local skip = entry.Skip or {} local be = entry.Be or false startRebuildWatcher(macro, i, skip, be) break end end end

-- 📂 Load macro local config = getgenv().TDX_Config or {} local macroName = config["Macro Name"] or "event" local macroPath = "tdx/macros/" .. macroName .. ".json" globalPlaceMode = config["PlaceMode"] or "normal"

-- Ánh xạ lại tên mode if globalPlaceMode == "unsure" then globalPlaceMode = "rewrite" elseif globalPlaceMode == "normal" then globalPlaceMode = "ashed" end

if not isfile(macroPath) then error("Không tìm thấy macro file: " .. macroPath) end
...
-- ▶️ Chạy macro như thường lệ for _, entry in ipairs(macro) do if entry.TowerPlaced and entry.TowerVector and entry.TowerPlaceCost then local vecTab = entry.TowerVector:split(", ") local pos = Vector3.new(unpack(vecTab)) local args = { tonumber(entry.TowerA1), entry.TowerPlaced, pos, tonumber(entry.Rotation or 0) } WaitForCash(entry.TowerPlaceCost) PlaceTowerRetry(args, pos.X, entry.TowerPlaced) elseif entry.TowerUpgraded and entry.UpgradePath and entry.UpgradeCost then local axisValue = tonumber(entry.TowerUpgraded) UpgradeTowerRetry(axisValue, entry.UpgradePath) elseif entry.ChangeTarget and entry.TargetType then local axisValue = tonumber(entry.ChangeTarget) ChangeTargetRetry(axisValue, entry.TargetType) elseif entry.SellTower then local axisValue = tonumber(entry.SellTower) SellTowerRetry(axisValue) end end

-- 🚨 Kích hoạt rebuild nếu có SuperFunction DetectRebuildEntry(macro)
