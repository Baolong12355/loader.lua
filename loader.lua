...
-- load webhook.lua để log inventory
spawn(function()
    pcall(function()
...
-- chạy các script theo config
spawn(function() tryRun(player.Name, "Join Map", getgenv().TDX_Config["Map"] ~= nil, links["Join Map"]) end)

-- luôn chạy CTG, bất kể place id
spawn(function() tryRun(player.Name, "CTG", getgenv().TDX_Config["CTG"], links["CTG"]) end)
