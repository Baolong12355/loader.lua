-- TDX Macro Runner - Rebuild (Fixed, Be Logic, Vector Split Fix)
...
local skipMap = {} -- ghi lại những tên tower được skip nếu be=true
local macroRun = {}
local placedTowers = {}
...
for i, line in ipairs(macro) do
    ...
    elseif line.SuperFunction == "rebuild" then
        ...
        if line.Be then
            for j = 1, i - 1 do
                local pre = macro[j]
                if pre.TowerPlaced then
                    skipMap[tonumber((pre.TowerVector:split(","))[1])] = skipSet[pre.TowerPlaced]
                end
            end
        end
        task.spawn(function()
