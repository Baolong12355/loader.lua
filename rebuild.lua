...
local function BuildTowerRecords(macro, maxLine, skipList, beMode)
    local records, skipSet, latestName = {}, {}, {}
...
-- 🚀 Rebuild Watcher chính
function startRebuildWatcher(macro, maxLine, skipList, beMode)
    local records = BuildTowerRecords(macro, maxLine, skipList, beMode)
    LogDebug("[WATCHER] Bắt đầu theo dõi tower cần rebuild...")
...
local function DetectRebuildEntry(macro)
    for i, entry in ipairs(macro) do
        if entry.SuperFunction == "rebuild" then
            ...
            LogDebug("[REBUILD] Gặp dòng rebuild tại #"..i.." → skip:", table.concat(skip, ", "))
...
LogDebug("✅ Đã chạy xong macro & khởi động rebuild nếu có.")