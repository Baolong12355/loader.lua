...
local function createGUI()
    ...
    local M2Toggle = MainTab:CreateToggle({
        ...
        Callback = function(Value)
            if Value then
                if not table.find(combatSettings.selectedSkills, "MOUSEBUTTON2") then
                    table.insert(combatSettings.selectedSkills, "MOUSEBUTTON2")
                end
            else
                local index = table.find(combatSettings.selectedSkills, "MOUSEBUTTON2")
                if index then
