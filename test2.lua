...
---------------------------------------------------------------------------------
--- CÁC HÀM TIỆN ÍCH & HỖ TRỢ (UTILITY & HELPER FUNCTIONS)
---------------------------------------------------------------------------------
...
-- Tìm tower bằng tọa độ X (sử dụng so sánh tuyệt đối theo yêu cầu)
local function GetTowerByAxis(targetX)
    for hash, tower in pairs(TowerClass.GetTowers()) do
        local scf = tower.SpawnCFrame
        if scf and typeof(scf) == "CFrame" and scf.Position.X == targetX then
            return hash, tower
        end
    end
    return nil, nil
...
-- Chờ cho đến khi tower không chỉ tồn tại mà còn được khởi tạo đầy đủ
...
---------------------------------------------------------------------------------
--- CÁC HÀM THỰC THI HÀNH ĐỘNG (ACTION EXECUTION FUNCTIONS)
---------------------------------------------------------------------------------
...
---------------------------------------------------------------------------------
--- HỆ THỐNG REBUILD CHÍNH (MAIN REBUILD SYSTEM)
---------------------------------------------------------------------------------
task.spawn(function()
    ...
    ---
    --- LUỒNG 1: THỰC THI (REBUILD WORKER)
    ---
        ...
        task.spawn(function()
            while true do
                if #jobQueue > 0 then
                    ...
                    if placeRecord and not GetTowerByAxis(job.x) then
    ...
    ---
    --- LUỒNG 2: XÁC MINH (VERIFICATION WORKER)
    ---
    ...
    ---
    --- VÒNG LẶP CHÍNH: GIÁM SÁT & TẠO JOB (PRODUCER)
    ---
    while true do
        -- Tải lại macro nếu có thay đổi
        ...
        -- Tạo cache tower hiện có để quét nhanh
        ...
        -- Sắp xếp lại toàn bộ hàng đợi nếu có job mới được thêm vào
